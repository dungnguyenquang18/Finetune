...
def cut_cot_from_decoded(decoded: str) -> str:
    #  Tìm phần <cot>...</cot>
    cot_match = re.search(r"<cot>(.*?)</cot>", decoded, re.DOTALL)
    cot_text = cot_match.group(1).strip() if cot_match else ""

    # Phần đầu ra cuối (sau </cot>)
    after_cot = re.split(r"</cot>", decoded, maxsplit=1)
    final_output = after_cot[1].strip() if len(after_cot) > 1 else ""

    # Gộp vào trường mới
    return "<cot>\n" + cot_text + "<\cot>" + "\n" + final_output if cot_text else final_output
...
def evaluate(
    ...
    for src, ref in tqdm(list(zip(sources, references)), desc="Generating"):
        ...
        outputs_detail.append({
            ...
            "hypothesis": tgt,
            "cot_and_output": cut_cot_from_decoded(decoded_full),
        })
...
def main():
    ...
    parser.add_argument('--adapter_dir', type=str, default='Finetune/finetune/qwen3_envi_lora_cot')
    parser.add_argument('--data_file', type=str, default='Finetune/data/.json')
    parser.add_argument('--max_new_tokens', type=int, default=666)
    parser.add_argument('--beam', type=int, default=5)
    parser.add_argument('--out', type=str, default='test_cot_finetune.json')
